...
module.exports = {
  showBanner: (text, font) => uiManager.showBanner(text, font),
  createSpinner: (text, options) => uiManager.createSpinner(text, options),
  logInfo: (message, fromSpinner) => uiManager.logInfo(message, fromSpinner),
  logError: (message, fromSpinner) => uiManager.logError(message, fromSpinner),
  logSuccess: (message, fromSpinner) => uiManager.logSuccess(message, fromSpinner),
  logWarning: (message, fromSpinner) => uiManager.logWarning(message, fromSpinner),
  logDebug: (message, fromSpinner) => uiManager.logDebug(message, fromSpinner),
  stopAllSpinners: () => uiManager.stopAllSpinners(),
  setLogLevel: (level) => uiManager.setLogLevel(level),
  // Export other methods as they are implemented, e.g.:
  // displayTable: (data, headers) => uiManager.displayTable(data, headers),
  // confirm: (message, defaultValue) => uiManager.confirm(message, defaultValue),
  displayTable: (data, headers) => uiManager.displayTable(data, headers),
  displayStats: (stats) => uiManager.displayStats(stats),
  displayAudioStatus: (audioStats) => uiManager.displayAudioStatus(audioStats),
  createProgressBar: (total, options) => uiManager.createProgressBar(total, options),
  showLoading: (message, duration) => uiManager.showLoading(message, duration),
  confirm: (message, defaultValue) => uiManager.confirm(message, defaultValue),
  clear: () => uiManager.clear(),
  sectionHeader: (title, emoji) => uiManager.sectionHeader(title, emoji),
  summary: (stats, emoji) => uiManager.summary(stats, emoji),
  showCustomBanner: (main, sub) => uiManager.showCustomBanner(main, sub),
}; 